...
import { DismissRegular, AttachRegular } from '@fluentui/react-icons';
import {
  useCallback,
  useEffect,
  useRef,
...
/**
 * TODO
 * - [ ] deleting one file should delete only that file (requires some key on the file to iterate it to avoid key issues in the list with react)
 */
export default function Component() {
  ...
  return (
    <div className={styles.fileUpload}>
      ...
      <Caption1>File uploader w/ fancy background</Caption1>
    </div>
  );
}
...
function useFileUpload({ maxFiles }: FileUploadOptions) {
  ...
  const fileChanged = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (files) {
        setFiles(Array.from(files).slice(0, maxFiles));
      }
    },
    [setFiles, maxFiles],
  );
  ...
  useEffect(() => {
    ...
    input.multiple = maxFiles ? maxFiles > 1 : false;
    input.disabled = false;

    if (maxFiles !== undefined && files && files.length >= maxFiles) {
      input.disabled = true;
    }
  }, [files, maxFiles]);
